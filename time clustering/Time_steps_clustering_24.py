...
def algorithm(data,name,value,solarweight,windweight,hydroweight,demandweight,mid_night,path):
    ...
    panda.to_excel(path+'/24 Time Slices (Seasonal).xlsx')

    panda = pd.DataFrame(solar_and_wind_sorter_seasonal.T)
    panda.to_excel(path+'/24 Time Slices Sorted (Seasonal).xlsx')

    panda = pd.DataFrame(solar_and_wind_duration_curve_seasonal.T)
    panda.to_excel(path+'/Duration Curve (Seasonal).xlsx')

    panda = pd.DataFrame(solar_and_wind_annual.T)
    panda.to_excel(path+'/24 Time Slices (Annual).xlsx')

    panda = pd.DataFrame(solar_and_wind_sorter_annual.T)
    panda.to_excel(path+'/24 Time Slices Sorted (Annual).xlsx')

    panda = pd.DataFrame(solar_and_wind_duration_curve_annual.T)
    panda.to_excel(path+'/Duration Curve (Annual).xlsx')
    ...
    while region < total_regions:
        ...
        while s_w_h_d < 4:
            ...
            plt.savefig(path+'/24 Time slices '+ name[region]+' seasonal '+s_w_h_ds[s_w_h_d])
    ...
    while region < total_regions:
        ...
        plt.savefig(path+'/24 Time slices '+ name[region]+' annual '+s_w_h_ds[s_w_h_d])
