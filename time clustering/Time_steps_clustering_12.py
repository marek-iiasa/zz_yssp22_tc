# -*- coding: utf-8 -*-
"""
This script calculates representative time slices for different number of time slices.
"""
...
def algorithm(
    ...
    while region < total_regions:
        ...
        while w < 13 * 2:
            ...
            while d1 < 7:
                ...
                while x2 < 2190 + 2190:
                    sum1 = sum1 + (
                        (solar_load_curve_Wi[x2] - s_w_4_sorted[int(x2 / 365 / 2)]) ** 2
                    )  # Inverno
                    sum3 = sum3 + (
                        (solar_load_curve_Su[x2] - s_su_4_sorted[int(x2 / 365 / 2)])
                        ** 2
                    )  # Verao

                    sum1 = sum1 + (
                        (wind_load_curve_Wi[x2] - w_w_4_sorted[int(x2 / 365 / 2)]) ** 2
                    )
                    sum3 = sum3 + (
                        (wind_load_curve_Su[x2] - w_su_4_sorted[int(x2 / 365 / 2)]) ** 2
                    )

                    sum1 = (
                        sum1
                        + (
                            (hydro_load_curve_Wi[x2] - h_w_4_sorted[int(x2 / 365 / 2)])
                            ** 2
                        )
                        / 3
                    )  # Hydropower is not well represented in the model, so we reduce the impact of hydropower on the final results.
                    sum3 = (
                        sum3
                        + (
                            (hydro_load_curve_Su[x2] - h_su_4_sorted[int(x2 / 365 / 2)])
                            ** 2
                        )
                        / 3
                    )
                    ...
                    sum1 = sum1 + (
                        (demand_load_curve_Wi[x2] - d_w_4_sorted[int(x2 / 365 / 2)])
                        ** 2
                    )
                    sum3 = sum3 + (
                        (demand_load_curve_Su[x2] - d_su_4_sorted[int(x2 / 365 / 2)])
                        ** 2
                    )
                    x2 = x2 + 1
                ...
                if sum1 != 0 and sum1 < w_minimum[0]:
                    ...
                    demand_results_seasonal[0:6] = d_w_4_sorted1[
                        0 + d1 * 6 : 6 + d1 * 6
                    ]
                ...
                if sum3 != 0 and sum3 < su_minimum[0]:
                    ...
                    solar_results_seasonal[6:12] = s_su_4_sorted1[
                        0 + d1 * 6 : 6 + d1 * 6
                    ]
                    wind_results_seasonal[6:12] = w_su_4_sorted1[
                        0 + d1 * 6 : 6 + d1 * 6
                    ]
                    hydro_results_seasonal[6:12] = h_su_4_sorted1[
                        0 + d1 * 6 : 6 + d1 * 6
                    ]
                    demand_results_seasonal[6:12] = d_su_4_sorted1[
                        0 + d1 * 6 : 6 + d1 * 6
                    ]
        ...
        solar_results_sorted_annual = np.sort(solar_results_sorted_seasonal, axis=None)[
            ::-1
        ]
        wind_results_sorted_annual = np.sort(wind_results_sorted_seasonal, axis=None)[
            ::-1
        ]
        hydro_results_sorted_annual = np.sort(hydro_results_sorted_seasonal, axis=None)[
            ::-1
        ]
        demand_results_sorted_annual = np.sort(
            demand_results_sorted_seasonal, axis=None
        )[::-1]

        solar_and_wind_seasonal = np.vstack(
            (
                solar_and_wind_seasonal,
                solar_results_seasonal,
                wind_results_seasonal,
                hydro_results_seasonal,
                demand_results_seasonal,
            )
        )
        solar_and_wind_sorter_seasonal = np.vstack(
            (
                solar_and_wind_sorter_seasonal,
                solar_results_sorted_seasonal,
                wind_results_sorted_seasonal,
                hydro_results_sorted_seasonal,
                demand_results_sorted_seasonal,
            )
        )
        solar_and_wind_duration_curve_seasonal = np.vstack(
            (
                solar_and_wind_duration_curve_seasonal,
                solar_load_curve_seasonal,
                wind_load_curve_seasonal,
                hydro_load_curve_seasonal,
                demand_load_curve_seasonal,
            )
        )

        solar_and_wind_annual = np.vstack(
            (
                solar_and_wind_annual,
                solar_results_annual,
                wind_results_annual,
                hydro_results_annual,
                demand_results_annual,
            )
        )
        solar_and_wind_sorter_annual = np.vstack(
            (
                solar_and_wind_sorter_annual,
                solar_results_sorted_annual,
                wind_results_sorted_annual,
                hydro_results_sorted_annual,
                demand_results_sorted_annual,
            )
        )
        solar_and_wind_duration_curve_annual = np.vstack(
            (
                solar_and_wind_duration_curve_annual,
                solar_load_curve_annual,
                wind_load_curve_annual,
                hydro_load_curve_annual,
    ...
    while region < total_regions:
        ...
        while s_w_h_d < 4:
            ...
            ax.plot(
                x_axis[1:7],
                np.sort(solar_and_wind_sorter_seasonal[n, 0:6], axis=None)[::-1],
                color="b",
            )
            ...
            ax2.plot(
                x2_axis[1 : 2191 + 2190],
                solar_and_wind_duration_curve_seasonal[n, 0 : 2190 + 2190],
                color="r",
            )
            ...
            ax.plot(
                x_axis[1:7],
                np.sort(solar_and_wind_sorter_seasonal[n, 6:12], axis=None)[::-1],
                color="b",
            )
            ax.set_xlabel("Time Slices")
            ax.set_ylabel("%")
